...
gst_pylonsrc_class_init (GstPylonsrcClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_SERIAL,
      g_param_spec_string ("serial", "serial", "(String) Camera Serial Number as defined by Basler's API. If this parameter is not specified the first device which is connected will be used. If there are multiple cameras and this parameter is specified then the camera with the serial number matching the serial property will be opened. In case this parameter is specified and it does not match the serial number of the list of available cameras then it will generate an error and plugin will not run.", "",
  ...
  g_object_class_install_property (gobject_class, PROP_IMAGEFORMAT,
      g_param_spec_string ("imageformat", "Image format", "(YUV422_YUYV_Packed/Mono8/Bayer8/Bayer10/Bayer10p/RGB8/BGR8/YCbCr422_8). Determines the pixel format in which to send frames. Note that downstream elements might not support some of these.", "Bayer8",
          (GParamFlags) (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));
...
gst_pylonsrc_init (GstPylonsrc *pylonsrc)
{
  ...
  pylonsrc->height = 0;
  pylonsrc->width = 0;
  pylonsrc->interpacketdelay = 1500;
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (property_id) {
    case PROP_SERIAL:
      pylonsrc->serial = g_value_dup_string(value+'\0');
      break;
    case PROP_CAMERA:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (property_id) {
    case PROP_SERIAL:
      g_value_set_string(value, pylonsrc->serial);
      break;
    case PROP_CAMERA:
  ...
  if(!deviceConnected) {
    ...
    if(strncmp("bayer", pylonsrc->imageFormat, 5) == 0) {
      ...
      if(strcmp(pylonsrc->imageFormat, "rgb8") == 0) {
        ...
        format = "GRAY8\0";
      } else if (strcmp(pylonsrc->imageFormat, "yuv422") == 0) {
        format = "YUY2\0";
      }
    }
...
gst_pylonsrc_start (GstBaseSrc * src)
{
  ...
  if(numDevices==0) {
    ...
    GST_DEBUG_OBJECT(pylonsrc, "Multiple cameras found, and the user didn't specify which camera to use.");
    GST_DEBUG_OBJECT(pylonsrc, "Please specify the camera using the serial property.");
    GST_DEBUG_OBJECT(pylonsrc, "The camera IDs are as follows: ");

    for(i=0; i<numDevices; i++) {
      PYLON_DEVICE_HANDLE deviceHandle;
      PylonDeviceInfo_t pDi;
      res = PylonGetDeviceInfo(i, &pDi);
      GST_DEBUG_OBJECT(pylonsrc, "%s", pDi.SerialNumber);
      GST_DEBUG_OBJECT(pylonsrc, "Serial Number: %s.", pylonsrc->serial);

      if (strcmp(pDi.SerialNumber, pylonsrc->serial)==0 && res == GENAPI_E_OK){
  ...
  PYLONC_CHECK_ERROR(pylonsrc, res);
  GST_DEBUG_OBJECT(pylonsrc, "Setting resolution to %" PRId64 "x%" PRId64 ".", pylonsrc->width, pylonsrc->height);

  if(PylonDeviceFeatureIsAvailable(pylonsrc->deviceHandle, "GevSCPD")) {
    res = PylonDeviceSetIntegerFeature(pylonsrc->deviceHandle, "GevSCPD", pylonsrc->interpacketdelay);
    PYLONC_CHECK_ERROR(pylonsrc, res);
  } else {
    GST_WARNING_OBJECT(pylonsrc, "This camera doesn't support changing inter packet delay");
  }
  ...
  if(strncmp("bayer", pylonsrc->imageFormat, 5) == 0) {
    ...
    if(PylonDeviceFeatureIsAvailable(pylonsrc->deviceHandle, "EnumEntry_PixelFormat_YUV422_YUYV_Packed")) {
      g_string_printf(pixelFormat, "YUV422_YUYV_Packed");
    } else {
      GST_ELEMENT_ERROR(pylonsrc, RESOURCE, FAILED, ("Failed to initialise the camera"), ("Camera doesn't support YUV422Packed"));
      goto error;
    }

  } else {
    GST_ERROR_OBJECT(pylonsrc, "Invalid parameter value for imageformat. Available values are: bayer8, bayer10, bayer10p, rgb8, bgr8, ycbcr422_8, mono8, yuv422. Value provided: \"%s\".", pylonsrc->imageFormat);
    GST_ELEMENT_ERROR(pylonsrc, RESOURCE, FAILED, ("Failed to initialise the camera"), ("Invalid parameters provided"));
    goto error;
  }
  GST_DEBUG_OBJECT(pylonsrc, "Using %s image format.", pixelFormat->str);
  ...
  GST_DEBUG_OBJECT(pylonsrc, "Initialised successfully.");
  return TRUE;
...
static GstFlowReturn gst_pylonsrc_create (GstPushSrc *src, GstBuffer **buf)
{
  ...
  grabResult.Context = 0;
  grabResult.pBuffer = 0;
  grabResult.hBuffer = 0;
  grabResult.Status = 0;
  _Bool bufferReady;
  GstMapInfo mapInfo;
...
while(grabResult.Status != Grabbed){
  ...
  PYLONC_CHECK_ERROR(pylonsrc, res);
  if(!bufferReady) {
    GST_MESSAGE_OBJECT(pylonsrc, "Couldn't get a buffer from the camera");
    goto error;
  }
}
  ...
  *buf = gst_buffer_new_and_alloc(pylonsrc->payloadSize);
  ...
  return GST_FLOW_OK;
error:
  GST_MESSAGE_OBJECT(pylonsrc, "GST flow error");
    ...
    if (strcmp(pylonsrc->serial,"\0")==0){
        ...
        for(i = 0; i < numDevices; i++){
          ...
          if (strcmp(pylonsrc->serial,"\0")==0){
            ...
            PylonDeviceInfo_t pD;
            res = PylonGetDeviceInfo(pylonsrc->cameraId, &pD);
            res = PylonCreateDeviceByIndex(pylonsrc->cameraId, &pylonsrc->deviceHandle);
            GST_DEBUG_OBJECT(pylonsrc, "%s", pD.SerialNumber);
            GST_DEBUG_OBJECT(pylonsrc, "Serial Number: %s.", pylonsrc->serial);
            if (strcmp(pD.SerialNumber, pylonsrc->serial)==0){
              res = PylonDeviceOpen(pylonsrc->deviceHandle, PYLONC_ACCESS_MODE_CONTROL | PYLONC_ACCESS_MODE_STREAM);
              PYLONC_CHECK_ERROR(pylonsrc, res);
              deviceConnected = TRUE;
              return TRUE;
